...
> 打开本地目录 → 文件树管理 → 多标签编辑，内置**引用机制、模板机制、文档校验、批注评论、Mermaid 图表**等一整套内容工程能力。
...
| 批注 | 选中文本 → 工具条「添加批注」/ `Ctrl+R` | 评论线程、回复、标记已解决 |
...
│   ├── src/
│   │   ├── editor/     #   多标签管理 + 引用机制 + Mermaid + 源码模式
...
│   │   ├── state/      #   全局状态 / 设置 / 文件树操作
│   │   ├── components/ #   FileTree / TabBar / EditorPane / 设置 / 抽屉…
│   │   └── App.vue     #   布局：侧边栏 + 标签栏 + 编辑器 + 状态栏
│   ├── src-tauri/      #   Rust 壳（文件系统命令 / 窗口 / 打包配置）
│   ├── tests/e2e/      #   Playwright 端到端回归套件
│   └── docs/design.md  #   里程碑设计文档（M1-M6 完整实现记录）
...
- 改代码后必跑回归：`npm run test:e2e`（12 个套件，需 dev server :5173），完成后 `npm run build` 验证。
