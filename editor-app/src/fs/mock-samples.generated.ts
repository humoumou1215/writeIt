...
### ④ 块级增删（04-块级增删.md）
- 纯新增段（整段绿）、纯删除段、引用块删除。
- 列表项新增 \`- 丙\`：**保留 \`- \` marker**（\`- {++丙++}\`），独立成列表项、不吞入上一行，且**有「新增了此段」批注卡并正确连线**（旧版多余行不产卡）。

### ⑤ mermaid 流程图（05-mermaid流程图.md）
- **二元语义（M16）：只有红（删除）与绿（新增）**，无黄色「修改」标注。
- **C 删除＝红框虚线+划线**、**F 新增＝绿**；D 节点仅标签文字变化 → 不标注不打扰。
- 批注卡：\`流程图：新增 1 个节点、删除 1 个\`；点击卡 → 连线 + 滚动定位。

### ⑥ mermaid 时序图（06-mermaid时序图.md）
- 消息文本标注二元：**新增绿 / 删除红（划线）**；删除消息以原行加回图底部红标（可见被删消息）。
- 精确文本匹配（\`推送客户\` 不会误命中 \`推送客户资料\`）；批注卡 \`新增 2 个消息、删除 1 个\`。

### ⑦ mermaid 状态图（07-mermaid状态图.md）
- 状态节点标注二元：PENDING/FINISHED 绿（新增）、WAIT/LENDING 红（删除），无黄色修改。

### ⑧ 嵌入引用与摘要（08-嵌入引用与摘要.md）
- **二元语义（M16）**：移除的引用以**缩略红行**展示（\`移除引用：[[path]]\` 1 行，非卡片）+「移除了引用」批注卡；新增的引用为**卡片 + 「新增引用」绿徽标** +「新增了引用」批注卡。
- 不再出现「引用变更」（黄）概念。
- 说明：源文件「内容有改动」摘要依赖工作区脏文件，请用下图「工作区场景」验证。
...
export const DEMO_HASH = '15w6nin'
