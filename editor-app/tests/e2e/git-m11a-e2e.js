...
// 5. 源码模式三态互斥：diff 内 Ctrl+E = 模式循环（渲染↔文本分栏↔文本统一）；退出 diff 后 Ctrl+E 仍切源码
await L.press('Control+e')
await L.waitMs(400)
C.check('diff 内 Ctrl+E 留在 diff（文本分栏）', (await L.q('.diff-row.split')) > 0)
...
// M11d 还原 → 右键菜单「还原整个文件到 HEAD」
await L.rightClick('.git-diff-view', 0)
await L.waitMs(300)
C.check('M11d 右键菜单含整文件还原', (await L.has('[class*="menu"] .menu-item', '还原整个文件到 HEAD')))
await L.clickText('[class*="menu"] .menu-item', '还原整个文件到 HEAD')
await L.waitMs(400)
