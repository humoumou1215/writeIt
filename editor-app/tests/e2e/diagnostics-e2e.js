...
await L.waitFor(() => L.vis('.settings-modal'), 2000)
await L.clickText('.settings-modal .tab-btn', '高级')
await L.waitFor(async () => (await L.qText('.settings-modal .group-title', '问题诊断')) > 0, 2000)
...
const diagBtnInSettings = await js(`[...document.querySelectorAll('.settings-modal button')].some(b => (b.textContent || '').includes('打开诊断'))`)
