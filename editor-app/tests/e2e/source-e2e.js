...
await L.waitFor(async () => (await L.txt('.ProseMirror')).includes('第一段内容'), 4500)

// 1. Ctrl+E 进入源码模式
await L.press('Control+e')
await L.waitFor(activeTaShown, 3000)
...
// 3. Ctrl+E 切回所见即所得
await L.press('Control+e')
await L.waitFor(async () => !(await activeTaShown()) && (await L.txt('.ProseMirror')).includes('源码编辑'), 4000)
...
await L.waitFor(async () => (await L.q('.tab .dot.dirty')) === 0, 4000)
...
await L.type('\n\n保存前加的源码内容。')
await L.press('Control+s')
await L.waitFor(async () => (await L.q('.tab .dot.dirty')) === 0, 5000)
...
// 8. inline-code 改绑 Mod+Shift+E（macOS Chromium 中 Mod=Meta；Windows 中 Mod=Control）
const boxp = await js(`(() => {
...
await L.waitMs(300)
await L.press('Meta+Shift+e')
await L.waitFor(async () => (await L.q('.ProseMirror code')) > 0, 3000)
C.check('Mod+Shift+E 切换行内代码（改绑生效）', (await L.q('.ProseMirror code')) > 0)
const mdInline = await L.pageMd()
C.check('行内代码写入 markdown（反引号）', mdInline.includes('`'))
await L.focusEditor()
await L.press('Control+e'); await L.waitFor(activeTaShown, 3000)
