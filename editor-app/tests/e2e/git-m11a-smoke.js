...
const clickChangeFile = async (name) => {
  await gitBtn()
  let ok = await js(`!![...document.querySelectorAll('.git-panel .scm-row')].find(x=>(x.textContent||'').includes(${JSON.stringify(name)}))`)
  if (!ok) {
    await L.clickText('.git-panel .section-title', '更改')
    await L.waitMs(200)
    ok = await js(`!![...document.querySelectorAll('.git-panel .scm-row')].find(x=>(x.textContent||'').includes(${JSON.stringify(name)}))`)
  }
  if (!ok) throw new Error('更改区未找到 ' + name)
  await L.clickText('.git-panel .scm-row .name', name)
}
...
if ((await L.attr('.view-toggle', 'title')) === '树形') {
  await L.clickEl('.view-toggle', 0, { label: '切换平铺视图' })
  await L.waitMs(200)
}
...
await L.waitFor(() => L.qText('.branch, .bp-row', 'feature/图表优化') >= 1, 3000)
C.check('分支区含 feature/图表优化', (await L.qText('.branch, .bp-row', 'feature/图表优化')) >= 1)
await L.clickEl('.branch-badge', 0, { label: '关闭分支列表' })
C.check('工作区 3 文件（README/需求表/会议纪要）', (await L.qText('.section .ws-file, .section .scm-row', 'README')) + (await L.qText('.section .ws-file, .section .scm-row', '会议纪要')) + (await L.qText('.section .ws-file, .section .scm-row', '需求表')) >= 3)
...
await clickChangeFile('README')
await L.waitFor(() => L.q('.git-diff-view') === 1 && L.q('.render-host') === 1, 8000)
C.check('进入 diff 视图', (await L.q('.git-diff-view')) === 1)
C.check('默认渲染模式（render-host）', (await L.q('.render-host')) === 1)
...
await L.waitMs(1500)
C.check('M14 mermaid 修改节点（新节点绿 + 旧值红删除双节点）', (await L.qText('.render-host svg g.node.diff-node-add, .render-host svg g.node.diffAdd', '授信成功')) >= 1 && (await L.qText('.render-host svg g.node.diff-node-del, .render-host svg g.node.diffDel', '支付成功')) >= 1)
C.check('M14 mermaid 新增节点（diff-node-add）', (await L.q('.render-host svg g.node.diff-node-add, .render-host svg g.node.diffAdd')) >= 1)
C.check('M14 mermaid 删除节点（diff-node-del）', (await L.q('.render-host svg g.node.diff-node-del, .render-host svg g.node.diffDel')) >= 1)
...
C.check('M14 嵌入改动提示', (await L.q('.render-host .ref-file-block')) >= 2 && (await L.qText('.annotation-drawer .ad-card .ad-card-title', '改动说明')) >= 5)
...
// 4. 会议纪要
await ensureSidebar()
await clickChangeFile('会议纪要')
await L.waitFor(() => L.q('.render-host') === 1 && L.qText('.render-host .diff-ins', '消息通知需求收集') > 0, 10000)
...
// 5. 需求表
await ensureSidebar()
await clickChangeFile('需求表')
await L.waitFor(() => L.q('.render-host') === 1 && L.qText('.render-host .diff-ins', '消息通知') > 0, 10000)
...
await ensureSidebar()
await clickChangeFile('README')
await L.waitFor(() => L.q('.git-diff-view') === 1, 5000)
// Ctrl+E：渲染 → 文本分栏
await L.press('Control+e')
await L.waitMs(400)
C.check('切文本模式（分栏）', (await L.q('.diff-row.split')) > 0)
