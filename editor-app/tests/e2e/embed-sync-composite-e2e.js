...
await L.waitFor(async () => {
  ...
  return blocks.length === 4 && blocks.some((x) => x.ro && x.txt.includes(SEED))
}, 6000)
const readonlySeedSnapshot = (await allBlocks()).find((x) => x.ro)?.txt || ''
await openTab('composite-b.md', '打开宿主B')
await L.waitFor(async () => (await blockTexts())[0]?.includes(SEED), 5000)
...
C.check('A 块1 追加成功', /^inserted@\d+$/.test(a1 || ''))
await L.waitMs(2200)
...
const dead = ab.find((x) => !x.ro && !x.col && x.txt.includes('引用失败：找不到文件「不存在源XYZ」'))
C.check('只读块存在且可编辑块已变', !!roB)
C.check('只读块未被广播污染（保持打开宿主时快照）', roB ? roB.txt === readonlySeedSnapshot && !roB.txt.includes('A块一改C2') : false)
C.check('断链块只显示失败态、不物化源内容', !!dead)

// ================= 复合 CS3：链式嵌套级联 =================
await openTab('l1.md', '打开链L1')
await L.waitFor(async () => (await blockTexts())[0]?.includes('CHAIN-源文本'), 6000)
...
await L.waitMs(300)
await L.type('\n链源编辑级联X')
await L.waitFor(async () => (await L.pageMd()).includes('链源编辑级联X'), 3000)
await L.clickText('.tab', 'l1', { label: '切链L1(2)' })
await L.waitFor(async () => (await blockTexts())[0]?.includes('链源编辑级联X'), 5000)
const l1AfterCascade = await blockTexts()
C.check('链式级联：L1 块含链源新内容', l1AfterCascade[0]?.includes('链源编辑级联X'))
...
C.check('折叠块只显示治理提示、不物化源正文', cyc.filter((x) => x.col).every((x) => x.txt.includes('循环引用') && !x.txt.includes('# 环A')))
// 编辑别处，折叠块不被广播展开
...
await L.clickText('.tab', '数据库字段引用', { label: '切源保存(并发前)' })
await L.waitMs(900)
await js(`window.__saveActiveTab()`)
await L.waitMs(2200)
...
// 并发块编辑：模型层下两块为同一源投影（收敛一致）；steps 跨 schema 回退旧路时
// 可能整串覆盖（spec §11 R1 已知边界）。安全断言：编辑不静默全丢。
const anyC1 = cc.some((t) => t.includes('并发块1AAA'))
const anyC2 = cc.some((t) => t.includes('并发块2BBB'))
C.check('并发编辑至少保留一条（不静默全丢）', cc.length === 2 && (anyC1 || anyC2))
await js(`window.__saveActiveTab()`)
await L.waitMs(900)
const dconf = await diskOf(SRC)
C.check('保存写回包含至少一条并发编辑', dconf.includes('并发块1AAA') || dconf.includes('并发块2BBB'))
