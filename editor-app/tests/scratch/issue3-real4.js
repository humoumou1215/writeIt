...
(async () => {
  ...
  // goEnd 后输入内容（自动滚动跟随），光标应可见
  await page.evaluate(() => window.__editorGoEnd());
  await page.waitForTimeout(300);
  await page.keyboard.type('尾部内容');
  await page.waitForTimeout(500);
  const cur = await page.evaluate(() => {
    const sel = window.getSelection();
    const r = sel.rangeCount ? sel.getRangeAt(0).getBoundingClientRect() : null;
    return r ? { top: Math.round(r.top), vh: window.innerHeight, onScreen: r.top > 0 && r.top < window.innerHeight - 20 } : null;
  });
  console.log('输入后光标:', JSON.stringify(cur));
