...
(async () => {
  ...
  // 进入 笔记 → 选择 会议记录（第 2 个文件）→ 实体级 → ← 返回
  await page.keyboard.press('Enter');  // 笔记
  await page.waitForTimeout(500);
  await page.keyboard.press('ArrowDown');  // 会议记录
  await page.keyboard.press('Enter');  // 进入实体级
  ...
  console.log('← 返回后 hover:', JSON.stringify(back));
  const ok = back.hover && back.hover.includes('会议记录');
  console.log(ok ? '✅ 回到上次选中的会议记录' : '❌ 未恢复到会议记录');
  await browser.close();
})();
