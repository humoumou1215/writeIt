...
- **用户决策（评审后）：** 选择 B — 接受当前 Embed controlled harness + workspace/reference `Refresh` retry 作为当前 acceptance 证据；真实 production loader failure/retry journey 记录为后续 deferred/test gap，不再作为当前 P4 gate blocker
- **最终 Gate（用户决策落地后）：** **PASS**
- **P5 状态：** P5 尚未开始；P4 gate 已允许进入下一阶段，`P5-01` blocker 已解除（按 SPEC 先执行 `P5-00`；本次不启动 P5）
...
初始 re-review 在用户决策前因 production evidence gap 记录为 HOLD；现按用户决策 B 落地为 PASS。F-01 与 F-02 的独立 remediation 已在后续 follow-up 中关闭：
...
3. **Embed production acceptance gap：已按用户决策 B 接受为当前 gate 证据。** Embed controller 的 generation/retry/late-result 协议已有证据，当前 controlled harness + workspace/reference `Refresh` retry 被接受；真实 persistence loader failure/retry production journey 仍记录为后续 deferred/test gap，不阻断当前 P4 gate。

目录操作的当前批准策略仍然是“影响已打开 descendant Document/path binding 时阻止”，用户已确认继续沿用该 fail-closed 策略；本次不把它改成 transactional path migration。该策略足以保护当前运行时 path binding 的一致性，但不等于完整 directory reference migration；未打开 descendant 的 incoming-reference rewrite、path rebind、rollback 仍是明确 deferred 风险。
...
额外只读扫描未发现 v2 source 的 `editor-app` runtime import、Core 到 UI/platform/editor 的越层 import 或以 `setTimeout`/`setInterval`/`sleep`/固定 delay 作为状态协议的命中。测试通过证明当前 fixture/路径满足合同，不表示真实 native adapter、directory migration 或独立 production Embed loader failure/retry 用户旅程已经闭合；后者按用户决策 B 作为当前 gate 的非阻断 deferred/test gap 保留。
...
本次及用户决策均不改变该批准策略；若未来要实现完整 migration，必须另行提出架构/用户决策，不在后续 Task 中局部绕过。
...
### F-03 — 用户决策 B 已接受；production loader failure/retry 为 deferred/test gap
...
**用户决策 B（已落地）：** 接受当前 Embed controlled harness + workspace/reference `Refresh` retry 作为本次 acceptance 证据。真实 production loader failure/retry journey 只记录为后续 deferred/test gap，不再作为当前 P4 gate blocker；本次文档整理不实现 remediation。
...
这是已批准 conservative policy 的边界，且用户已确认继续接受“open descendant block + closed reference migration deferred”。不能在 STATUS 中写成 directory rename/move 已全面安全；若未来需要完整 path migration，必须另立 Task/ADR，不能在后续任务中静默改变策略。
...
### 当前必须返工后才能关闭的 blocker

- 无。F-03 原始 production acceptance finding 已按用户决策 B 重新分类为非阻断 deferred/test gap；因此当前没有必须返工后才能关闭的 P4 blocker。
...
- App production Embed failure/retry wiring（真实 loader rejection journey）；controlled harness + workspace/reference `Refresh` retry 已按用户决策 B 接受为当前 gate 证据。
...
- P2-06 richer Markdown、P8 template catalog/doctype、P7 Git provenance、office clipboard matrix/large-table performance 仍是后续 deferred 项；P5 Table Engine 已获进入授权但尚未开始，不应被本次 P4 gate 的 PASS 混入为已完成。

## 7. 文档与状态一致性

本次复核发现并在本评审中处理以下 drift：

- `writeit-v2/README.md` 仍写成“下一任务为 P3-08”，与已完成 P4-R05 不一致；已同步为用户决策 B 后的 P4 gate PASS，明确 P5 尚未开始，下一建议按 SPEC 为 P5-00。
- `STATUS.md` 原本 `Blocked: None` 且 `Next: P5-01`，会错误授权 P5；已同步为 P4-AR2 PASS，解除 P5-01 gate blocker，同时不把 P5 产品实现写成已开始。
...
3. **Embed acceptance（决策 B）：** 已接受当前 Embed controlled harness + workspace/reference `Refresh` retry 作为本次 acceptance 证据；真实 production loader failure/retry journey 是后续 deferred/test gap，不再阻断当前 P4 gate。
4. **目录策略：** 已确认继续采用“有打开 descendant 时阻止移动；完整 directory migration deferred”的 fail-closed 策略；如未来改为 transactional migration，必须另立架构决策。
5. P2A 真实 IME/用户验收记录仍是后续 release-quality evidence 项；历史 P2A-AR1 PASS 不被自动化 test count 取代，但本项不重新打开当前 P4 gate。

本次没有遗留必须重新确认的 gate 决策；后续若扩大目录迁移或 production acceptance 范围，再按新 Task/ADR 流程单独决策。
...
- 仍没有真实 App 的 persistence loader rejection → 用户可见 Embed failure → 明确 Refresh/retry → child mount 的完整 Chromium 旅程。当前 `ensureEmbeddedDocument()` 的失败恢复仍依赖 workspace/reference refresh 事件；按用户决策 B，该真实 journey 保留为后续非阻断 deferred/test gap，controlled harness + Refresh retry 则作为当前 acceptance evidence。F-03-R3 的 popup parity 和 F-03-R1/R2 的成功/dirty/image 路径不改变这一风险分类。
...
- **按用户决策 B**，当前 Embed controlled harness + workspace/reference `Refresh` retry 被接受为本次 acceptance 证据；真实 production loader failure/retry journey 保留为非阻断 deferred/test gap，不再阻断 P4 gate。
- `P5-01` 的 gate blocker 已解除，但 P5 产品实现尚未开始；按 SPEC，下一建议 Task 为 `P5-00`，本评审不自动开始下一 Task。
- 目录操作继续采用已批准的 fail-closed 策略：影响已打开 descendant 时阻止移动；完整 directory migration、closed-document incoming-reference rewrite、path rebind/rollback 仍 deferred。accepted ADR 与目录策略均未改变。
