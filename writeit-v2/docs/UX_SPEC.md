...
- 默认单一编辑区，Raw Source / Live Preview 在原编辑器内切换；独立只读对照预览仍按需打开，但不再通过可见的文档「···」菜单展示，具体入口由后续预览任务明确。
- 文件、搜索、Git 位于左侧；新建表单收纳在局部展开入口。大纲为独立可开关、可调宽左侧面板；反向引用和批注按需在右侧显示。
- 显示模式切换保留原快捷键，并在切换时用短暂提示反馈当前模式；Save 不作为常驻按钮展示，保存命令、快捷键、自动保存和状态反馈仍然保留。文档 `···` 菜单不再作为可见入口展示，低频文档操作不得因此破坏底层命令或数据安全，后续入口需由对应任务明确。
- 移除页面标题中的阶段号、projection/revision 展示与架构说明。设置采用浮层，避免推挤正文。
- 本次仅落实布局；大纲 active tracking、面板组合的更窄窗口策略仍需独立交互验收，不宣称完整 UX_SPEC 已全部实现。

### 3.1B 调试演示数据基线

调试页面的默认工作区不是“占位首页”，而是用于观察 WriteIt 主要交互的最小产品样本。默认演示数据需要让用户无需手工准备文件，就能检查：

- 常规 Markdown：多级标题、段落、强调、链接、引用、无序/有序/任务列表、代码块、表格、图片或附件占位、未知语法保留；
- 多文件夹和多标签导航，以及足够长的正文、长行和中英文混排；
- 多层 Embed/Reference、同一目标的重复 Embed、循环关系、断链、标题片段和只读 Embed；
- Outline 对组合内容的展示、引用/被引用关系及 Occurrence 角标、Search 命中、Raw/Live 切换、dirty/save 状态和右键菜单入口。

演示数据应使用稳定、可读的 Markdown 文件名和关系，方便用户反馈具体路径与引用序号。它属于浏览器调试 seed，不改变 Markdown/DocumentStore 权威，也不能替代 core、integration 和 browser 测试 fixtures；当产品功能新增时，相关演示场景应一并维护。
...
- “定位当前文件”不再作为工作区标题栏常驻按钮，迁移到文件树/工作区对象的右键菜单，并保持可发现、可键盘操作。
...
- reveal current file（定位当前文件）通过文件树/工作区对象的右键菜单提供，不在工作区标题栏常驻展示；仍须可发现、可键盘操作，并能展开祖先目录与滚动到当前文件。
...
Settings（设置）不要成为“所有实现细节的垃圾桶”。

设置面板的面向用户文案统一使用简体中文，包括标题、分组名、标签、按钮、说明、状态、空状态和错误提示。`commandId`、快捷键组合和必要的技术标识可以保留英文，但不能替代主要用户文案。

设置面板采用“左侧设置大纲 + 右侧设置内容”的结构：

- 左侧大纲列出当前确实可用的设置分组；没有内容的未来分组不提前显示为空壳；
- 点击左侧分组后，右侧滚动到对应区域并突出当前分组；
- 左侧大纲本身支持键盘聚焦和上下移动，不能只依赖鼠标；
- 设置面板打开/关闭、Tab 顺序、Escape 和焦点恢复仍遵守 UX-03 模态焦点规则；
- 右侧内容可以独立滚动，左侧大纲在滚动时保持可见。
...
首版只显示已经有设置内容的分组。工作区（工作区壳层）至少应包含：
...
- [x] “定位当前文件”迁移到文件树/工作区右键菜单；当前文件在 File Tree / Git 中默认轻量突出。
...
- [x] Save 不作为常驻可见按钮展示；保存命令、快捷键、自动保存和状态反馈仍保留。
- [x] 文档 `···` 菜单不再作为可见 UI 展示；其中低频能力不得在实现任务中被静默删除，必须由对应任务明确替代入口或分类。
- [x] 设置面板的用户可见文案使用简体中文，并采用左侧设置大纲导航当前可用分组。
