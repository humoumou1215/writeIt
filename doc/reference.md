...
**复制来源两种（行为一致）**：左侧文件树右键「复制」；或系统文件管理器复制（Windows 资源管理器 / macOS Finder）。

**编辑区 Ctrl+V**：剪贴板含复制的文件时，直接粘贴为 **链接引用** `[[path]]`（默认），不再进入普通文本粘贴。

- 系统复制：tauri 下把绝对路径转成工作区相对路径；工作区外/浏览器模式退化为文件名（Obsidian 式全库匹配，找不到则断链红标）。
- 复制**目录**：粘贴其路径文本（不作为引用）。
- 复制**多个文件**（系统多选）：连续插入多个引用，自动分段（每段一个）。

**编辑器右键菜单**（替代浏览器默认菜单）：

| 分组 | 菜单项 | 说明 |
|---|---|---|
| 粘贴组（剪贴板有复制文件） | 粘贴为链接引用 / 粘贴为块嵌入 / 粘贴为只读嵌入 | 三种方式可复用同一次复制，多次右键重复粘贴 |
| 常规编辑 | 粘贴 Ctrl+V / 剪切 Ctrl+X / 复制 Ctrl+C | 点击执行原生剪贴板动作 |
| 引用操作（右键在引用上） | 打开引用 / 复制引用 `[[path]]` | 跳转目标 / 复制整段引用语法 |
| 类型切换（右键在引用上） | 链接引用 / 块嵌入（可编辑）/ 只读嵌入 | 当前类型打 ✓，切换即生效（块嵌入自动物化） |

**实现**：`editor-menu.ts`（$prose 插件，捕获阶段监听 contextmenu/paste）+ `clipboard-core.ts`（内部剪贴板 + 自定义 MIME `application/x-writeit-node` + uri-list 解析 + 插入/切换事务）+ `RefEditorMenu.vue`（菜单 UI）。
...
| `editor-menu.ts` + `RefEditorMenu.vue` | 文件引用粘贴（Ctrl+V）+ 编辑器右键菜单（粘贴组/编辑组/引用操作/类型切换） |
| `clipboard-core.ts` | 内部剪贴板 + 自定义 MIME + uri-list 解析 + 插入/切换事务 |
